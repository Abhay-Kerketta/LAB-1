...
router.get('/read',(req,res)=>{
  const data=fs.readFile("techno.json","utf-8",(err,data)=>{
    if(err) throw err;
...
fs.readFile("techno.json","utf-8",(err,data)=>{
    if(err){
        console.log(err);
    }
    else{
        const json=require('./techno.json');
         json.records.push(data1);
         fs.writeFile("techno.json",JSON.stringify(json),err=>{
            if(err) throw err;
         });
         res.send(json);
    }
